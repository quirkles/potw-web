...
import {Maybe, PartialNull} from "@/utils/typeUtils";

type IAuthUser = Maybe<Partial<{
    email: string;
    firestoreId: string;
    sqlId: string;
}>>
...
export const authUserSlice = createSlice({
    ...
    reducers: {
        initializeAuthUser: (state: IAuthUser, action: PayloadAction<IAuthUser>) => {
        ...
        setAuthUserField: <T extends keyof IAuthUser>(state: IAuthUser, action: PayloadAction<{field: T, value: IAuthUser[T]}>) => {
            if(state !== null) {
