...
function GameListItem(props: PropsWithChildren<{game: StoreGame}>) {
    ...
    return (
        <StyledGameItem>
            <h3>
                <Link href={`games/${game.id}`} style={{textDecoration: "underline", cursor: "pointer"}}>{game.name}</Link>
            </h3>
            <p>{props.game.description}</p>
            <div>
                <h4>
                    <Link href={`users/${game.admin.sqlId}`} style={{textDecoration: "underline", cursor: "pointer"}}>{game.admin.username || game.admin.email}</Link>
                </h4>
            </div>
        </StyledGameItem>
    )
